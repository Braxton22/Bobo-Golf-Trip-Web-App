...
export default async function CourseEditPage({ params }: PageProps) {
  ...
  if (!user) redirect(`/login?next=/admin/course/${courseId}`);
  ...
  const totalPar = holes.reduce((acc, x) => acc + x.par, 0);

  return (
    <AdminSection
      title={course.name}
      description="Course basics, 18 holes (par & stroke index), tees & yardages."
      back={{ href: "/admin/course" }}
    >
      {/* Course info ----------------------------------------------------- */}
      <section className="card space-y-3">
        <h2 className="font-medium">Course info</h2>
        <form action={updateCourseInfoAction} className="space-y-3">
          <input type="hidden" name="course_id" value={course.id} />
          <Field label="Course name">
            <input className="input" name="name" required defaultValue={course.name} />
          </Field>
          <FormRow>
      ...
      <section className="card space-y-3">
        ...
        <ul className="space-y-2">
          ...
          {tees.length === 0 && <li className="text-sm text-muted-foreground">No tees yet.</li>}
        </ul>
      ...
      {/* 18 holes — par, stroke index, per-tee yardages ------------------ */}
      <section className="card space-y-3">
              ...
              <tbody>
                {holes.map((hole) => (
                  <tr key={hole.id} className="border-t border-line">
                    <td className="py-2 pr-2 font-medium tabular-nums">{hole.hole_number}</td>
                    ...
                    <td className="py-2 pr-2">
                      <input
                        ...
                        min={1}
                        max={18}
                        name={`si_${hole.hole_number}`}
                        defaultValue={hole.stroke_index}
                        inputMode="numeric"
                      />
                    </td>
                    {tees.map((t) => {
                      const teeYards = yardages.find(
                        (y) => y.tee_id === t.id && y.hole_id === hole.id
                      )?.yards;
                      return (
                        <td key={t.id} className="py-2 pr-2">
                          <input
                            ...
                            min={50}
                            max={800}
                            name={`yards_${t.id}_${hole.hole_number}`}
      ...
      <section className="card space-y-2">
        <h2 className="font-medium">Delete course</h2>
        <p className="text-xs text-muted-foreground">
          Removes this course, its holes, tees, and yardages. Days assigned to it
          become unassigned.
        </p>
        <form action={deleteCourseAction}>
          ...
          <button
            type="submit"
            className="btn-ghost inline-flex items-center gap-1.5 text-xs text-destructive"
          >
            <Trash2 className="h-3.5 w-3.5" /> Delete {course.name}
          </button>
        </form>
      </section>
    </AdminSection>
